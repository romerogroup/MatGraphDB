...
class ElementNodes(NodeStore):
    def __init__(self, storage_path: str, base_element_file=BASE_ELEMENT_FILE):
        ...
        super().__init__(
            ...
            initialize_kwargs={"base_element_file": base_element_file},
        )
    ...
    def initialize(self, base_element_file="imputed_periodic_table_values.parquet"):
        ...
        try:
            ...
            if file_ext == "parquet":
                ...
                df = pd.read_csv(
                    os.path.join(PKG_DIR, "utils", base_element_file), index_col=0
                )
            else:
            ...
            df["experimental_oxidation_states"] = df[
                "experimental_oxidation_states"
            ].apply(lambda x: eval(x))
            df["ionization_energies"] = df["ionization_energies"].apply(
                lambda x: eval(x)
            )
