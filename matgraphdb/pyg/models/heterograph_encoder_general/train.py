...
original_train_data["materials"].node_ids = parent_data["materials"].node_ids[
    train_dict["materials"]
]
original_train_val_data = parent_data.subgraph(train_val_dict)
original_train_val_data["materials"].node_ids = parent_data["materials"].node_ids[
    train_val_dict["materials"]
]
original_test_data = parent_data.subgraph(test_dict)
original_test_data["materials"].node_ids = parent_data["materials"].node_ids[
    test_dict["materials"]
]
original_test_val_data = parent_data.subgraph(test_val_dict)
original_test_val_data["materials"].node_ids = parent_data["materials"].node_ids[
    test_val_dict["materials"]
]
...
for split_label, data in split_data.items():
    data["elements", "rev_has", "materials"].edge_label_index = data[
        "materials", "has", "elements"
    ].edge_label_index[[1, 0]]
    data["elements", "rev_has", "materials"].edge_label = data[
        "materials", "has", "elements"
    ].edge_label
    data["space_groups", "rev_has", "materials"].edge_label_index = data[
        "materials", "has", "space_groups"
    ].edge_label_index[[1, 0]]
    data["space_groups", "rev_has", "materials"].edge_label = data[
        "materials", "has", "space_groups"
    ].edge_label
    data["crystal_systems", "rev_has", "materials"].edge_label_index = data[
        "materials", "has", "crystal_systems"
    ].edge_label_index[[1, 0]]
    data["crystal_systems", "rev_has", "materials"].edge_label = data[
        "materials", "has", "crystal_systems"
    ].edge_label
...
def eval_metrics(preds, targets, **kwargs):
    ...
    auc_score = roc_auc_score(
        targets.cpu().detach().numpy(), preds.cpu().detach().numpy()
    )
...
def pca_plots(
    embeddings_per_node_type, n_nodes_per_node_type, node_labels_per_node_type, pca_dir
):

    os.makedirs(pca_dir, exist_ok=True)
    # 3. Combine embeddings only for the selected node types
    z_all = np.concatenate(
        [embeddings for embeddings in embeddings_per_node_type.values()], axis=0
    )
...
def plot_learning_curves(results, save_path, measure="mae"):
    ...
    for idx, split in enumerate(results):
        ...
        if measure not in split_data:
            print(
                f"Warning: Measure '{measure}' not found for split '{split}'. Skipping."
            )
            continue
...
metrics_per_rel_per_split = defaultdict(
    lambda: defaultdict(lambda: {value_name: [] for value_name in values_to_record})
)
node_types = test_val_data.metadata()[0]
n_nodes_per_split_per_node_type = defaultdict(
    lambda: defaultdict(lambda: {node_type: [] for node_type in node_types})
)
...
for split_name, data_batch in split_data.items():
    for node_type in data_batch.metadata()[0]:
        n_nodes_per_split_per_node_type[split_name][node_type] = data_batch[
            node_type
        ].num_nodes
        if node_type != "materials":
            node_labels_per_split_per_node_type[split_name][node_type] = data_batch[
                node_type
            ].labels
    ...
    if epoch % CONFIG.training.eval_interval == 0:
        ...
        for split_name, data_batch in split_data.items():
            ...
            for rel_name in rel_names:
                ...
                metrics_per_rel_per_split[rel_name][split_name][
                    "current_loss"
                ] = rel_loss
                metrics_per_rel_per_split[rel_name][split_name][
                    "current_predictions"
                ...
                for metric_name, metric_value in metrics.items():
                    metrics_per_rel_per_split[rel_name][split_name][metric_name].append(
                        metric_value
                    )
...
plot_learning_curves(
    results, save_path=os.path.join(run_learning_curve_dir, "learning_curves.png")
)
plot_learning_curves(
    results_original,
    save_path=os.path.join(run_learning_curve_dir, "learning_curves_original.png"),
)
...
for metric_name in metric_names:
    ...
    for rel_name in rel_names:
        ...
        total_save_path = os.path.join(
            rel_run_learning_curve_dir, f"{metric_name}_learning_curve.png"
        )
