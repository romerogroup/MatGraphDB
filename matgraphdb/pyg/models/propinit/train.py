...
def build_heterograph():
    ...
    builder.add_node_type("crystal_systems", drop_null=True, label_column="crystal_system")
    ...
    if DATA_CONFIG.create_random_features:
        ...
        heterodata["materials"].x = torch.normal(
            mean=0.0,
            std=1.0,
            size=(n_materials, DATA_CONFIG.n_material_dim)
        )
...
def partition_and_convert_graph(source_data):
    ...
    node_partitions = metis(
        rowptr=rowptr,
        col=col,
        num_partitions=PROPINET_CONFIG.model.n_partitions
    )
...
def train_linear_baseline(split_data):
    ...
    for epoch in range(num_epochs):
        ...
        x = split_data["train"]["materials"].original_x[split_data["train"]["materials"].y_split_index]
        ...
        with torch.no_grad():
            for split, data_batch in split_data.items():
                model.eval()
                x_eval = data_batch["materials"].original_x[data_batch["materials"].y_split_index]
...
def train_propinet(split_data, heterodata):
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=PROPINET_CONFIG.training.learning_rate)
    loss_fn = nn.L1Loss()
