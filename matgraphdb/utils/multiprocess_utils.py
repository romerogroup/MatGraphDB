...
def get_cpus_per_node():
    ...
    if cpu_per_node is None:
        ...
        logger.debug("SLURM_JOB_CPUS_PER_NODE contains multiple nodes with variable CPUs per node.")
        cpu_per_node, num_nodes = cpu_per_node.strip(')').split('(x')
        cpus_node_list = [int(cpu_per_node) for _ in range(int(num_nodes))]
    else:
        logger.debug("SLURM_JOB_CPUS_PER_NODE contains a multiples node with fixed CPUs per node.")
