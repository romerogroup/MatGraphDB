...
class ParquetDatabase:
    ...
    def _filter_data(self, df, filter_tuple):
        filter_string= "x['{}'] {} {}".format(filter_tuple[0], filter_tuple[1], filter_tuple[2])
        filtered_df = df[df['data'].apply(lambda x: eval(filter_string))]
        return filtered_df
...
def test_filter_func(df):
    filtered_df = df[df['data'].apply(lambda x: x['band_gap'] == 1.593)]
    return filtered_df
...
if __name__ == '__main__':
    ...
    # df=db.read(table_name='main', filter_data_func=test_filter_func)

    df=db.read(table_name='main', filter_arg=('band_gap',  '==', 1.593))
