...
class PolyhedronModel(nn.Module):
    ...
    def __init__(self, n_gc_layers:int=1, n_neurons:List=[n_node_features]):
        ...
        self.relu = nn.ReLU()
        n_neurons = 5
        self.l1 = nn.Linear(n_node_features,n_neurons)
        self.l2 = nn.Linear(n_neurons*2, 1)
    ...
    def forward(self, x, targets=None):
        ...
        # out = out_1+out_2
        if torch.randint(1, 100, (1,),device=device) >50:
            out = torch.cat((out_1, out_2), 1)
        else:
            out = torch.cat((out_2, out_1), 1)
        # print(out.shape)    
    ...
    def generate_polyhedron_enocoding(self, x):
    ...
    def predict(self, x):
        ...
        # out = out_1+out_2 
        # print(torch.randint(1,101,1,device=device) )
        print(torch.randint(1, 100, (1,),device=device) >50)
        if torch.randint(1, 100, (1,),device=device) >50:
            out = torch.cat((out_1, out_2), 1)
        else:
            out = torch.cat((out_2, out_1), 1)
...
def main():
    ...
    if load_checkpoint:
        ...
        model = PolyhedronModel(n_gc_layers = 2, n_neurons=[n_node_features])
        m = model.to(device)
    ...
    # Training Looop
    if train_model:
        factor = float(n_train) / float(n_test)
        n_epoch_0 = 0
        model.train()
