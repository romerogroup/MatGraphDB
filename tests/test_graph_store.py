...
from matgraphdb.stores import NodeStore, EdgeStore
...
def test_data():
    ...
    node_2_type = 'item'
    edge_data = [
        {"source_id": 0, 'source_type': node_1_type, "target_id": 0, 'target_type': node_2_type, "weight": 0.5},
        {"source_id": 1, 'source_type': node_2_type, "target_id": 1, 'target_type': node_1_type, "weight": 0.7},
        {"source_id": 0, 'source_type': node_1_type, "target_id": 1, 'target_type': node_2_type, "weight": 0.5},
        {"source_id": 1, 'source_type': node_2_type, "target_id": 0, 'target_type': node_1_type, "weight": 0.7},
    ]
    edge_type = "test_edge"
...
def test_initialize_graphdb(graphdb):
    ...
    assert os.path.exists(graphdb.edge_generators_path), "Edge generators directory not created."
...
def test_add_node_type(graphdb):
    ...
    assert store == graphdb.node_stores[node_type], "Node store not retrieved correctly."
...
def test_add_nodes(graphdb):
    ...
    assert node_type_1 in graphdb.node_stores, "Node type 1 not registered in node_stores."
    assert node_type_2 in graphdb.node_stores, "Node type 2 not registered in node_stores."
    
    node_store_1 = graphdb.get_node_store(node_type_1)
    node_store_2 = graphdb.get_node_store(node_type_2)
    
    assert len(node_store_1.read_nodes()) == len(data_1), "Incorrect number of nodes added."
    assert len(node_store_2.read_nodes()) == len(data_2), "Incorrect number of nodes added."
...
def test_add_node_store(tmp_dir):
    ...
    with pytest.raises(ValueError, match=f"Node store of type {temp_store.node_type} already exists"):
...
def test_nodes_persist_after_reload(tmp_dir):
    ...
    assert initial_nodes.to_pydict()['name'] == ["Node1", "Node2"], "Incorrect node names."
    ...
    assert node_type in new_graph.node_stores, "Node type not registered in node_stores."
...
def test_node_exists(graphdb):
    ...
    assert not graphdb.node_exists("nonexistent_type"), "Non-existent node type should not exist."
...
def test_add_edge_type(graphdb):
    ...
    assert store == graphdb.edge_stores[edge_type], "Edge store not retrieved correctly."

def test_add_edges(graphdb, test_data):
    """Test adding edges."""
    # First create some nodes that the edges will connect
    nodes_1_data, node_1_type, nodes_2_data, node_2_type, edge_data, edge_type = test_data
    graphdb.add_nodes(node_1_type, nodes_1_data)
    graphdb.add_nodes(node_2_type, nodes_2_data)
    
    assert node_1_type in graphdb.node_stores, "Node type 1 not registered in node_stores."
    assert node_2_type in graphdb.node_stores, "Node type 2 not registered in node_stores."
...
def test_remove_edge_store(graphdb, test_data):
    ...
    assert edge_type not in graphdb.edge_stores, "Edge type not removed from edge_stores."
    assert not os.path.exists(os.path.join(graphdb.edges_path, edge_type)), "Edge store directory not removed."
    
    # Verify that trying to get the removed store raises an error
    with pytest.raises(ValueError, match=f"Edge store of type {edge_type} does not exist"):
...
def test_edge_generator_persistence(tmp_dir, element_store):
    ...
    assert table.shape == (391, 12), "Generator for element_element_neighborsByGroupPeriod edge output shape is incorrect"
