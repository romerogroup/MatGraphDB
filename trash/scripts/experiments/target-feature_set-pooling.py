...
def main():
    ...
    project_dir = os.path.dirname(os.path.dirname(experiment_dir))
    ...
    experiment_params = {
        'y_val': ['dihedral_energy', 'moi', 'energy_per_verts'],
        'feature_set_index' : [0,2,3],
        'global_pooling_method' : ['mean','add','max'],
    }
    
    for feature_set_index in experiment_params['feature_set_index']:
        ...
        experiment_name = f'target value_feature_set_{feature_set_index}_{dataset}_experiment'
        ...
        for global_pooling_method in experiment_params['global_pooling_method']:
            for y_val in experiment_params['y_val']:
                print(f'feature_set:{feature_set_index}, pooling:{global_pooling_method}, target:{y_val}')
                ...
                with mlflow.start_run(experiment_id=experiment_id,run_name=run_name):
                    ...
                    for epoch in range(n_epochs):
                        ...
                        # Early Stopping OR stopping if learning falls below certain value
                        if es(model=model, val_loss=batch_val_loss,mape_val_loss=batch_val_mape) or current_lr < 1e-5:
                    ...
                    mlflow.log_param(f'feature_set',feature_set_index)
